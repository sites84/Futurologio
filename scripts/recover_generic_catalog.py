...
def main():
    ...
    # Build repair statements from every canonical recovery record, not only
    # records still generic in catalog.json. This repairs INVENTIONS already
    # created by users while leaving any non-generic custom invention untouched.
    repair_sources={}
    for p in sources:
        if not isinstance(p,dict): continue
        pid=norm(p.get('id'))
        if pid and 160 <= int(pid.rsplit('-',1)[1]) <= 175 if pid.startswith('produto-') and pid.rsplit('-',1)[1].isdigit() else False:
    ...
    for p in repair_sources.values():
        ...
        stmts.append("UPDATE INVENTIONS SET name='%s', category='%s', concept='%s', data='%s' WHERE json_extract(data,'$.source_id')='%s' AND json_extract(data,'$.what') LIKE '%s';" % (name,cat,concept,sql_quote(js),sid,generic_phrase))
