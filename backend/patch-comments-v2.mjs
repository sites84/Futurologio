...
if(path.startsWith('/api/comments/')&&request.method==='DELETE'){const au=await auth(request,env);if(!au)return json({ok:false,error:'Sessão necessária.'},401);const commentId=Number(path.split('/')[3]);if(!Number.isInteger(commentId)||commentId<1)return json({ok:false,error:'Comentário inválido.'},400);const c=await env.DB.prepare('SELECT id,user_id FROM COMMENTS WHERE id=?').bind(commentId).first();if(!c)return json({ok:false,error:'Comentário não encontrado.'},404);if(String(c.user_id)!==String(au.sub))return json({ok:false,error:'Você só pode apagar o seu próprio comentário.'},403);await env.DB.prepare('DELETE FROM COMMENTS WHERE id=?').bind(commentId).run();return json({ok:true})}
`;
