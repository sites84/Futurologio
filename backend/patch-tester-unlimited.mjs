...
// Keep the existing tester exception intact even after the daily-credit patch runs.
...
// The daily-credit patch inserts a protected consume route before the original route.
// Make that route return immediately for the tester, so no creation is consumed.
...
if(!s.includes(testerReturn)){
  const consumeMarker="/* DAILY_CREDIT_CONSUME */if(path==='/api/consume-creation'&&request.method==='POST'){";
  if(s.includes(consumeMarker)){
    s=s.replace(consumeMarker,consumeMarker+userLookup+testerReturn);
    // Remove the duplicated auth/user lookup that followed the insertion.
    const duplicate=userLookup+testerReturn+userLookup;
    s=s.replace(duplicate,userLookup+testerReturn);
  }else{
