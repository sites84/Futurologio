(()=>{
'use strict';
if(window.__FUTURO_PRODUCT_SOCIAL_V1)return;
window.__FUTURO_PRODUCT_SOCIAL_V1=true;
...
function login(){if(!currentUser()||!token()){window.openAuth?.('login');return false}return true}
...
function setLiked(id,v){localStorage.setItem(key(id),v?'1':'0')}
...
async function refresh(){const id=dbId();if(!id)return;const like=likedLocal(id);const b=document.getElementById('fpsLike');if(b){b.classList.toggle('liked',like);b.setAttribute('aria-pressed',String(like))}
 try{const r=await fetch(API+'/api/invention/'+encodeURIComponent(id),{cache:'no-store'});if(!r.ok)throw 0;const d=await r.json();const serverLiked=!!(d.liked??d.user_liked??d.liked_by_user);if(currentUser()&&['true','false'].includes(String(d.liked??d.user_liked??d.liked_by_user)))setLiked(id,serverLiked);const now=serverLiked||likedLocal(id);if(b){b.classList.toggle('liked',now);b.setAttribute('aria-pressed',String(now))}const lc=document.getElementById('fpsLikeCount');if(lc)lc.textContent=Number(d.likes??d.like_count??d.data?.likes??0);const cc=document.getElementById('fpsCommentCount');if(cc)cc.textContent=Number(d.comment_count??d.comments_count??(Array.isArray(d.comments)?d.comments.length:0));renderComments(normalizeComments(d))}catch{}
}
async function toggleLike(){if(!login())return;const id=dbId();if(!id)return;const b=document.getElementById('fpsLike');if(!b)return;const before=likedLocal(id);b.disabled=true;try{const r=await fetch(API+'/api/inventions/'+encodeURIComponent(id)+'/like',{method:'POST',headers:{authorization:'Bearer '+token(),'content-type':'application/json'},body:'{}'});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||'Não foi possível curtir.');const server=typeof d.liked==='boolean'?d.liked:!before;setLiked(id,server);b.classList.toggle('liked',server);b.setAttribute('aria-pressed',String(server));const lc=document.getElementById('fpsLikeCount');if(lc&&d.likes!=null)lc.textContent=Number(d.likes)}catch(e){alert(e.message||'Não foi possível curtir.')}finally{b.disabled=false}}
async function sendComment(){if(!login())return;const id=dbId(),input=document.getElementById('fpsCommentInput'),btn=document.getElementById('fpsCommentSend');if(!id||!input||!btn)return;const text=input.value.trim();if(!text){input.focus();return}btn.disabled=true;btn.textContent='Enviando…';try{const r=await fetch(API+'/api/inventions/'+encodeURIComponent(id)+'/comment',{method:'POST',headers:{authorization:'Bearer '+token(),'content-type':'application/json'},body:JSON.stringify({text})});const d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||'Não foi possível comentar.');input.value='';const list=document.getElementById('fpsCommentList');if(list){const c=d.comment||{text,username:currentUser()?.username||'Você'};list.insertAdjacentHTML('afterbegin',`<div class="fps-comment"><b>${esc(commentName(c))}</b><span>${esc(commentText(c)||text)}</span></div>`)}const cc=document.getElementById('fpsCommentCount');if(cc)cc.textContent=Number(cc.textContent||0)+1;window.FUTUROLOGIO_CHECK_REWARDS?.();setTimeout(()=>input.focus(),20)}catch(e){alert(e.message||'Não foi possível comentar.')}finally{btn.disabled=false;btn.textContent='Enviar'}}
function renderComments(arr){const list=document.getElementById('fpsCommentList');if(!list)return;list.innerHTML=arr.map(c=>`<div class="fps-comment"><b>${esc(commentName(c))}</b><span>${esc(commentText(c))}</span></div>`).join('');const cc=document.getElementById('fpsCommentCount');if(cc&&arr.length)cc.textContent=arr.length}
const css=document.createElement('style');css.textContent=`#futuroProductSocial{margin:15px 0 0;padding:14px 0 0;border-top:2px solid var(--ink)}.fps-actions{display:flex;gap:10px;flex-wrap:wrap}.fps-btn{border:1.5px solid var(--ink);border-radius:12px;padding:13px 18px;font-weight:900;cursor:pointer;background:#fff}.fps-like.liked{background:#ff5b55;color:#fff}.fps-comments{display:none;margin-top:12px;border:1.5px solid var(--ink);border-radius:14px;padding:12px;background:#fff}.fps-comments.open{display:block}.fps-comment-form{display:flex;gap:8px}.fps-comment-form input{flex:1;min-width:0;border:1.5px solid var(--ink);border-radius:10px;padding:11px;background:#fff;color:#171717;font:inherit}.fps-comment-form button{border:1.5px solid var(--ink);border-radius:10px;padding:10px 14px;background:#d8ff55;font-weight:900;cursor:pointer}.fps-comment{display:flex;flex-direction:column;gap:3px;margin-top:9px;padding:9px 10px;background:#f5f2ea;border-radius:9px}.fps-comment b{font-size:12px}.fps-comment span{font-size:13px;line-height:1.4;white-space:pre-wrap;overflow-wrap:anywhere}`;document.head.appendChild(css);
