...
function handle401(){window.openAuth?.('login')}
...
function creatorId(d){const p=product()||{};return d?.user_id||d?.creator_id||d?.author_id||d?.owner_id||d?.user?.id||d?.creator?.id||d?.author?.id||d?.owner?.id||p.user_id||p.creator_id||p.author_id||p.owner_id||p.user?.id||p.creator?.id||''}
function creatorName(d){const p=product()||{};return d?.username||d?.user?.username||d?.creator?.username||d?.author?.username||d?.owner?.username||p.username||p.user?.username||p.creator?.username||'criador'}
function updateCreator(d){const b=document.getElementById('fpsCreator');if(!b)return;const id=creatorId(d);if(!id){b.hidden=true;return}b.hidden=false;b.textContent='Ver perfil de @'+creatorName(d);b.onclick=()=>{location.href='./profile.html?user='+encodeURIComponent(String(id))}}
function make(){
