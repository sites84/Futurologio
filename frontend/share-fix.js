...
function loadRewards(){if(document.getElementById('futuroGamificationEvents'))return;const s=document.createElement('script');s.id='futuroGamificationEvents';s.src='./frontend/gamification-events.js?v=20260910a';document.head.appendChild(s)}
loadRewards();
...
async function nativeShare(){const id=dbId(),u=shareUrl(),d=productData(),title=d.name+' — FUTUROLOGIO™',cta='Crie seu produto absurdo você também!';const text=[title,d.desc,cta].filter(Boolean).join('\n\n');if(!navigator.share){await copy(u);return}try{let file=null;if(id)file=await getImageFile(id,d.name);if(file&&navigator.canShare?.({files:[file]})){await navigator.share({title,text,url:u,files:[file]})}else{await navigator.share({title,text,url:u})}logShare()}catch(e){if(e?.name!=='AbortError')await copy(u)}}
