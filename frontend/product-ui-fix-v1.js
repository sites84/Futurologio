...
function flow(){const card=document.querySelector('.game-image-card');if(!card)return;let d=document.getElementById('futuroImageFlow');
  ...
  if(d.dataset.ready==='1')return;d.dataset.ready='1';
  ...
  const links=[{n:'ChatGPT',h:'https://chatgpt.com/',bg:'#10a37f',c:'#fff'},{n:'Leonardo',h:'https://app.leonardo.ai/',bg:'#6d28d9',c:'#fff'},{n:'Grok',h:'https://grok.com/',bg:'#111',c:'#d8ff55'},{n:'Gemini',h:'https://gemini.google.com/app',bg:'#4285f4',c:'#fff'}];
  d.querySelector('#aiLinks').innerHTML=links.map(x=>'<a href="'+x.h+'" target="_blank" rel="noopener" style="display:flex;align-items:center;justify-content:center;min-height:40px;border-radius:10px;font-weight:900;text-decoration:none;background:'+x.bg+';color:'+x.c+'">'+x.n+'</a>').join('');
}
