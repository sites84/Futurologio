(()=>{
'use strict';
if(window.__FUTURO_PRODUCT_UI_FIX_V5)return;
window.__FUTURO_PRODUCT_UI_FIX_V5=true;
...
function arrange(){const result=document.getElementById('result');if(!result||result.classList.contains('hidden'))return;const panel=result.querySelector('.panel');const details=result.querySelector('.details');const image=result.querySelector('.game-image-card');const name=result.querySelector('#name');if(!panel||!details||!image||!name)return;
  ...
  const afterUpload=upload||details;
