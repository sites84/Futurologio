(() => {
  ...
  function imageUrl(p) {
    ...
    if (id) return `${API}/api/invention-image/${id}`;
    return raw || '';
  }
  ...
  style.id = 'futuro-product-visual-social';
  style.textContent = `
    .futuro-image-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:9px}
    .futuro-image-action{border:1.5px solid #405476;border-radius:10px;background:#0b1427;color:#dce6ff;padding:9px 12px;font-size:10px;font-weight:1000;cursor:pointer;text-transform:uppercase;letter-spacing:.04em}
    .futuro-image-action.primary{background:#d8ff55;color:#091007;border-color:#fff}
    .futuro-image-action:hover{border-color:#62e6ff;transform:translateY(-1px)}
    .game-image-card.has-image img{cursor:zoom-in}
    .futuro-image-error{display:grid;place-items:center;min-height:180px;border:1px dashed #6b4560;border-radius:10px;background:radial-gradient(circle at 50% 40%,#2a1831,#0b1120 68%);color:#aebbd2;text-align:center;padding:24px}
    .futuro-image-error strong{display:block;color:#d8ff55;margin-bottom:6px;font-size:15px}
    .futuro-image-error span{font-size:11px;line-height:1.5}
    .futuro-fullscreen{position:fixed;inset:0;background:rgba(2,5,12,.96);z-index:99999;display:none;align-items:center;justify-content:center;padding:20px;backdrop-filter:blur(8px)}
    ...
    .ft-social .ft-btn[data-platform="whatsapp"]{background:#25D366!important}.ft-social .ft-btn[data-platform="facebook"]{background:#1877F2!important}.ft-social .ft-btn[data-platform="x"]{background:#000!important}.ft-social .ft-btn[data-platform="instagram"]{background:linear-gradient(135deg,#833AB4,#E1306C,#FCAF45)!important}.ft-social .ft-btn[data-platform="threads"]{background:#111!important}.ft-social .ft-btn[data-platform="copy"]{background:#405476!important}.ft-social .ft-btn[data-platform="native"]{background:#d8ff55!important;color:#091007!important}
    @media(max-width:700px){.ft-social .ft-social-row{grid-template-columns:repeat(2,minmax(0,1fr))}.ft-social .ft-social-row .ft-btn[data-platform="native"]{grid-column:1/-1}}
  `;
