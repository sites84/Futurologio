(() => {
  ...
  function mount(card){
    if(card.querySelector('.game-ai-links'))return;
    card.querySelector('.game-style-row')?.remove();
    const description=card.querySelector('.game-prompt-card>p');
    if(description)description.textContent='O FUTUROLOGIO monta automaticamente o prompt. Use sempre o estilo oficial 3D Toy-Tech Absurdo para manter uma identidade visual única em todas as invenções. Crie a imagem na IA de sua preferência e depois envie o arquivo ao site.';
    const actions=card.querySelector('.game-prompt-actions');
    if(!actions)return;
