...
let data=[],shown=4,ready=false,rendering=false;
...
function render(catalog){const feed=document.getElementById('recentFeed'),more=document.getElementById('moreBtn');if(!feed)return;rendering=true;feed.dataset.futuroRecent='1';const items=data.slice(0,shown);feed.innerHTML=items.length?items.map(x=>card(x,catalog)).join(''):'<div class="empty-state">Ainda não há criações globais registradas.</div>';if(more){more.hidden=shown>=data.length||!data.length;more.onclick=()=>{shown=Math.min(shown+4,data.length);render(catalog)}}rendering=false;ready=true}
async function init(){try{const catalog=await fetch('./catalog.json?recentfix=1').then(r=>r.json()).catch(()=>[]);const r=await fetch(API+'/api/explore?mode=recent&limit=24',{cache:'no-store'});if(!r.ok)throw new Error();const d=await r.json();data=(d.items||[]).slice().sort((a,b)=>Number(b.id||0)-Number(a.id||0));const start=()=>{if(document.getElementById('recentFeed')){render(catalog);observe(catalog)}};start()}catch{}}
function observe(catalog){const feed=document.getElementById('recentFeed');if(!feed)return;new MutationObserver(()=>{if(rendering||feed.dataset.futuroRecent==='1')return;setTimeout(()=>render(catalog),0)}).observe(feed,{childList:true,subtree:true});}
init();
})();
