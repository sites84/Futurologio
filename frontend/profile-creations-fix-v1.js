...
function card(x){const href='./criar.html?db_invention='+encodeURIComponent(x.id)+(x.source_id?'&invention='+encodeURIComponent(x.source_id):'');return '<a class="pv2-card" href="'+href+'">'+(x.image_url?'<img src="'+esc(x.image_url)+'" alt="'+esc(x.name)+'">':'<div class="pv2-empty">SEM FOTO</div>')+'<div><b>'+esc(x.name)+'</b><small>'+esc(x.category||'')+'</small></div></a>'}
async function fill(){const grid=document.querySelector('.pv2-creations');if(!grid||!token())return;if(grid.querySelector('.pv2-card'))return;
  let items=[];
